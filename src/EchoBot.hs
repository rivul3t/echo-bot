...
data Config = Config
  { -- | A reply to the @help@ command
    ...
    -- | Max count of repetition that bot offers to user after command
    -- @/repeat@ 
    confMaxRepetition :: Int
  }
...
--  | isBadCommand h message        = handleHelpCommand h
...
handleSettingRepetitionCount h count = do
  ...
  hModifyState' h (\st -> st { stRepetitionCount = count} )
  pure $ [MessageResponse $ hMessageFromText h $ "Success"]
...
handleRepeatCommand h = do
  ...
  count <- fmap stRepetitionCount $ hGetState h
  let   range = [1..(confMaxRepetition $ hConfig h)] -- TODO add maxRepetitionCount to config
        title = T.replace "{count}" (T.pack $ show count) (confRepeatReply $ hConfig h)
        offer_list = MenuResponse title (zip range $ map SetRepetitionCountEvent range)
    in pure $ [offer_list]
...
respondWithEchoedMessage h message = do
  ...
  count <- fmap stRepetitionCount $ hGetState h 
  pure $ replicate count $ MessageResponse message
--    where count = fmap stRepetitionCount $ hGetState h 
